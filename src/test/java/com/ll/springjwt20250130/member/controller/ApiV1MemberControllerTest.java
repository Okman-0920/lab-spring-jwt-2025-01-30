        ...
        void t1() throws Exception {
                ResultActions resultActions = mvc // mvc: MockMvc 객체를 통해 HTTP 요청을 수행
                                ...
                                .perform(
                                                ...
                                                post("/api/v1/members/join")
                                                                // content: HTTP 요청 본문에 Json 데이터를 추가하고, 회원 가입 시 필요한 정보를
                                                                // json형태로 전달
                                                                .content("""
                                                                                {
                                                                                    "username": "usernew",
                                                                ...
                                                                .contentType(
                                                                                // JSON 데이터를 전달하고, UTF-8 인코딩을 사용하는 것으로
                                                                                // 지정
                ...
                resultActions // 테스트의 결과는 다음과 같기를 기대한다.
                                ...
                                .andExpect(jsonPath("$.msg")
                                                .value("%s님 환영합니다. 회원가입이 완료되었습니다.".formatted(member.getNickname())))
                                .andExpect(jsonPath("$.data").exists())
                                .andExpect(jsonPath("$.data.id").value(member.getId()))
                                .andExpect(jsonPath("$.data.createDate").value(Matchers
                                                .startsWith(member.getCreateDate().toString().substring(0, 25))))
                                .andExpect(jsonPath("$.data.modifyDate").value(Matchers
                                                .startsWith(member.getModifyDate().toString().substring(0, 25))))
        ...
        void t10() throws Exception {
                ...
                resultActions
                                ...
                                .andExpect(jsonPath("$.createDate").value(
                                                Matchers.startsWith(actor.getCreateDate().toString().substring(0, 25))))
                                .andExpect(jsonPath("$.modifyDate").value(
                                                Matchers.startsWith(actor.getModifyDate().toString().substring(0, 25))))
